...
CONFIG_DIR = Path("/tmp/lorenz_eval_layer_configs_round21_predict_steps")
OUT_ROOT = REPO / "outputs_abfull" / "eval_layer_predict_steps_round21"


VARIANTS = [
    {"name": "ridge7e-5_ratio0375_predict300_n30", "predict_steps": 300},
    {"name": "ridge7e-5_ratio0375_predict350_n30", "predict_steps": 350},
    {"name": "ridge7e-5_ratio0375_predict450_n30", "predict_steps": 450},
    {"name": "ridge7e-5_ratio0375_predict500_n30", "predict_steps": 500},
]
...
def main() -> None:
    ...
    for variant in VARIANTS:
        ...
        cfg["reservoir"]["predict_steps"] = variant["predict_steps"]
        ...
        row = {
            "name": variant["name"],
            "predict_steps": variant["predict_steps"],
            "score": score_summary(summary, values),
            "summary": summary,
        }
        row.update(bins(values))
        summaries.append(row)
        with (OUT_ROOT / "round21_predict_steps_summary.json").open("w", encoding="utf-8") as f:
